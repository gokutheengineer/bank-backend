...
const getMaxBalanceForCurrency = `-- name: GetMaxBalanceForCurrency :one
SELECT max(balance) FROM accounts
WHERE currency = $1
`

func (q *Queries) GetMaxBalanceForCurrency(ctx context.Context, currency string) (interface{}, error) {
	row := q.db.QueryRow(ctx, getMaxBalanceForCurrency, currency)
	var max interface{}
	err := row.Scan(&max)
	return max, err
}

const getMinBalanceForCurrency = `-- name: GetMinBalanceForCurrency :one
SELECT min(balance) FROM accounts
WHERE currency = $1
`

func (q *Queries) GetMinBalanceForCurrency(ctx context.Context, currency string) (interface{}, error) {
	row := q.db.QueryRow(ctx, getMinBalanceForCurrency, currency)
	var min interface{}
	err := row.Scan(&min)
	return min, err
}
