...
const listTransfersAmong = `-- name: ListTransfersAmong :many
SELECT id, from_account_id, to_account_id, amount, created_at FROM transfers
WHERE 
...
func (q *Queries) ListTransfersAmong(ctx context.Context, arg ListTransfersAmongParams) ([]Transfer, error) {
	rows, err := q.db.Query(ctx, listTransfersAmong,
		arg.FromAccountID,
		arg.ToAccountID,
		arg.Limit,
		arg.Offset,
...
const listTransfersOf = `-- name: ListTransfersOf :many
SELECT id, from_account_id, to_account_id, amount, created_at FROM transfers
WHERE 
...
func (q *Queries) ListTransfersOf(ctx context.Context, arg ListTransfersOfParams) ([]Transfer, error) {
	rows, err := q.db.Query(ctx, listTransfersOf, arg.FromAccountID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
